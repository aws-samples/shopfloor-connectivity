...
class NatsWriterConfiguration : BaseConfigurationWithMetrics(), Validate{
    ...
    private var _targets: Map<String, NatsTargetConfiguration> = emptyMap()
    val targets: Map<String, NatsTargetConfiguration>
        get() = _targets.filter { (it.value.targetType == NATS_TARGET) }
    ...
    companion object {
        const val NATS_TARGET = "NATS-TARGET"

        private val default = NatsWriterConfiguration()

        fun create(targets: Map<String, NatsTargetConfiguration> = default._targets,
                   ...
                   templatesConfiguration: TemplatesConfiguration? = default._templates): NatsWriterConfiguration {

            val instance = createBaseConfiguration<NatsWriterConfiguration>(
                name = name,
                version = version,
