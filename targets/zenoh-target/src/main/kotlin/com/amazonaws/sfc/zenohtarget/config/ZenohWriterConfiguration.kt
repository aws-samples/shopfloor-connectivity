...
@ConfigurationClass
class ZenohWriterConfiguration : BaseConfigurationWithMetrics(), Validate{


    @SerializedName(CONFIG_TARGETS)
    private var _targets: Map<String, ZenohTargetConfiguration> = emptyMap()
    val targets: Map<String, ZenohTargetConfiguration>
        get() = _targets.filter { (it.value.targetType == ZENOH_TARGET) }
    ...
    companion object {
        const val ZENOH_TARGET = "ZENOH-TARGET"

        private val default = ZenohWriterConfiguration()

        fun create(targets: Map<String, ZenohTargetConfiguration> = default._targets,
                   ...
                   templatesConfiguration: TemplatesConfiguration? = default._templates): ZenohWriterConfiguration {

            val instance = createBaseConfiguration<ZenohWriterConfiguration>(
                name = name,
                version = version,
