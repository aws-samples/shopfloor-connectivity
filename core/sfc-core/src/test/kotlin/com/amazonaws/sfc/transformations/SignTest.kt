...
class SignTest {
    ...
    fun `deserialize from json`() {
        ...
        assertEquals(Sign.create(), Sign.fromJson(Gson().fromJson(json, JsonObject::class.java)), "from $json")
    }
    ...
    fun `operator logic an return types`() {
        ...
        val sign = Sign.create()
        for (v in testValues) {
            val target = v.first
            val result = sign.apply(target)
