...
class TransformationOperatorNumericOperandTest {

    class TestOperand(operand: Number) : TransformationImpl<Number>(operand) {
    }

    @Test
    fun `can create instance`() {
        assertDoesNotThrow() {
            TransformationOperatorNumericOperand()
        }
    }

    @Test
    fun `deserialize numeric operand`() {
        val json = """
            {
                "Operand" : 1
            }"""
