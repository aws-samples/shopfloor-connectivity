...
class AcosTest {
    ...
    fun `deserialize from json`() {
        ...
        assertEquals(Acos.create(), Acos.fromJson(Gson().fromJson(json, JsonObject::class.java)))
    }

    @Test
    fun `operator logic and return types`() {
        val testValues = listOf<Pair<Number?, Number?>>(
            Pair(0.0.toFloat(), kotlin.math.acos(0.0.toFloat())),
            Pair(0.0, kotlin.math.acos(0.0)),
            Pair(0, kotlin.math.acos(0.0)),
            Pair(null, null),
            Pair(2.0, null))


        val acos = Acos.create()
        for (v in testValues) {
            val target: Number? = v.first
            val result = acos.apply(target)
