...
@ConfigurationClass
class RestConfiguration : SourceAdapterBaseConfiguration() {
    ...
    val restProtocolAdapters: Map<String, RestAdapterConfiguration>
        get() = _protocolAdapters.filter { it.value.protocolAdapterType == REST_ADAPTER }
    ...
    private fun validateAdapters() {
        restProtocolAdapters.values.forEach { it.validate() }
    }


    private fun validateSchedules() {
        schedules.forEach { schedule ->
            schedule.sources.filter { isRestSource(it) }.forEach { source ->
                validateScheduleInput(source, schedule)
            }
        }
    }

    private fun isRestSource(source: Map.Entry<String, ArrayList<String>>) =
        restProtocolAdapters[sources[source.key]?.protocolAdapterID]?.protocolAdapterType == REST_ADAPTER
    ...
    private fun validateInputChannels(
        ...
        source: RestSourceConfiguration?,
        schedule: ScheduleConfiguration
    ) {
        sourceChannelMap.value.forEach { channel ->
            ConfigurationException.check(
                ((channel == WILD_CARD) || source!!.channels.containsKey(channel)),
                "Channel item \"$channel\" for REST source \"${sourceChannelMap.key}\" in schedule \"${schedule.name}\" does not exist, existing channels are ${source!!.channels.keys}",
                "Schedule.$CONFIG_SCHEDULE_SOURCES",
                schedule.sources
            )
        }
    ...
    private fun validateAtLeastOneSource() {
        ConfigurationException.check(
            sources.isNotEmpty(),
            "Configuration must have 1 or more REST sources",
    ...
    companion object {
        const val REST_ADAPTER = "REST"

        private val default = RestConfiguration()

        fun create(sources: Map<String, RestSourceConfiguration> = default._sources,
                   protocolAdapters: Map<String, RestAdapterConfiguration> = default._protocolAdapters,
