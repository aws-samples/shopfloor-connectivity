SFC `fatjar` setup in Greengrass V2
=======================================
...
It uses the fatjar created by SFC that contains the SFC core, all adapters, targets and the dependencies.
...
## Step 1: Create a fatjar for SFC

Checkout SFC to your local directory. Then execute the following command to build a fatjar:
...
This will produce an executable fatjar in `build/libs` like `build/libs/sfc-fatjar-1.8.9.jar`.
...
Upload the fatjar e.g. `core/fatjar/build/libs/sfc-fatjar-1.8.9.jar` to the S3 bucket. Feel free to put it into a folder if you want. 
Please copy the S3 URL e.g. `s3://my-s3-bucket/sfc-fatjar-1.8.9.jar` which you will need in the next step.
  ...
  "Manifests": [
    {
      ...
      "Artifacts": [
        {
          "Uri": "s3://[REPLACE WITH YOUR S3 BUCKET]/sfc-fatjar-1.8.9.jar"
        }
      ]
    }
  ]
