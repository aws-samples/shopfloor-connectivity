...
---
### Psn
The Parameter Suspect Number (PSN) identifies specific signals within a PGN message. The PSN field can be configured in three ways:
- A single PSN to retrieve one specific signal value
- A comma-separated list of PSNs (e.g., "EngSpeed, ActualEnginePercentTorque" ) to retrieve multiple selected signals
- Left empty to retrieve all signals defined in the DBC file for the specified PGN

A PSN can be specified in one of three formats:
...
When multiple signals are retrieved (either through a comma-separated list or by leaving PSN empty), the values are returned in a structured format with each signal's value accessible by its name.
    ...
    },
    {
      ...
      "properties": {
        ...
        "Psn": {
          ...
          "description": " Parameter Suspect Number (PSN) as decimal number, hex string with '0x' prefix, or PSN name"
        }
      },
      "required": [
        "Pgn"
...
Engine startermode and speed seelcted from EEC1 using numeric values for PGN and PSN
