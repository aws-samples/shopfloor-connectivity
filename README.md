...
Next we will have to configure the SFC. This is done via a [configuration file](./docs/core/sfc-top-level-config.md) you can specify at execution time (e.g. `sfc-main -config example.json`) We are first defining following variables which we will then use in an example configuration file. The following configuration sets SFC up, to connect to a OPCUA-Server and forward it to the S3 Bucket in your AWS Account:
